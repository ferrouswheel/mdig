...
def process_options(argv):
    ...
    def hRunOptions():
        try:
            opts, args = getopt.getopt(argv[1:], "ht:nafmod:Dv",
                    ["help","time","no-null","all","force","monitor","overwrite","dir=","debug","verbose","o","v","q"])
    ...
    def hAdminOptions():
        try:
            opts, args = getopt.getopt(argv[1:], "horgm:Dv", ["help","remove","generate","move-mapset=","o","v","q"])
        except getopt.GetoptError:
            admin_usage()
            sys.exit(1)
    ...
    switch = { "run":hRunOptions, "analysis":hAnalysisOptions,
        "admin":hAdminOptions, "node":hNodeOptions, "web":hWebOptions }
...
def main(argv):
    ...
    switch = { "run":hRun, "analysis":hAnalysis, "admin":hAdmin, "web":hWeb }
    function = None
    if switch.has_key( mdig_config.action_keyword ):
        function = switch[mdig_config.action_keyword]
