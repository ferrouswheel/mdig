...
class ExperimentInstance:
	...
	def getProbabilityEnvelopes(self):
		if not self.isComplete:
			self.log.warning("Trying to obtain probability envelope for incomplete instance")
	...
	def areEnvelopesUpToDate(self, ls, start, end):
		...
		if previous_envelopes is None:
			# TODO possibly this should only include times that
			# are meant to have output
	...
	def updateProbabilityEnvelope(self, ls, start, end):
		...
		if not missing_envelopes: return
		...
		for l in ls:
			...
			for t in missing_envelopes: #range(start,end):
